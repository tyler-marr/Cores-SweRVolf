...
1. Install West (Zephyr's build tool) and the Zephyr SDK according to the official guidelines at https://www.zephyrproject.org

2. Create a West workspace in the same directory as the FuseSoC workspace by running

        west init

3. Add the SweRVolf-specific drivers and BSP with
...
4. Enter the directory of the application to build. Zephyr comes with a number of example applications in the samples directory (`$WORKSPACE/zephyr/samples`), e.g. `$WORKSPACE/zephyr/samples/basic/blinky` contains the Zephyr blinky example. From now on, the program to build and run will be called `$APP`
5. Build the code with `west build -b swervolf_nexys`
